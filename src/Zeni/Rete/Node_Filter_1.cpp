...
namespace Zeni::Rete {
  ...
  std::shared_ptr<Node_Filter_1> Node_Filter_1::Create(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> node_key) {
    ...
    //  network->send_disconnect_from_parents(network, job_queue);
  ...
  std::pair<Node::Node_Trie::Result, std::shared_ptr<Node>> Node_Filter_1::connect_new_or_existing_output(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> key, const std::shared_ptr<Node> child) {
    ...
    const auto key_symbol = std::dynamic_pointer_cast<const Node_Key_Symbol_1>(key);
    assert(key_symbol);
  ...
  Node::Node_Trie::Result Node_Filter_1::link_output(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> key, const std::shared_ptr<Node> child) {
    ...
    const auto key_symbol = std::dynamic_pointer_cast<const Node_Key_Symbol_1>(key_node);
    assert(key_symbol);
  ...
  void Node_Filter_1::receive(const Message_Token_Insert &message) {
    ...
    for (auto &output : snapshot.snapshot<FILTER_LAYER_OUTPUTS>())
      job_queue->give_one(std::make_shared<Message_Token_Insert>(output, message.network, sft, Node_Key_Symbol_1::Create(symbol), message.token));
  }
  ...
  void Node_Filter_1::receive(const Message_Token_Remove &message) {
    ...
    for (auto &output : snapshot.snapshot<FILTER_LAYER_OUTPUTS>())
      job_queue->give_one(std::make_shared<Message_Token_Remove>(output, message.network, sft, Node_Key_Symbol_1::Create(symbol), message.token));
  }
  ...
  void Node_Filter_1::insert_tokens(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> key, const std::shared_ptr<Node> child, const Filter_Layer_Snapshot snapshot) {
    const auto sft = shared_from_this();
    ...
    for (const auto &token : snapshot.snapshot<FILTER_LAYER_TOKENS>())
      job_queue->give_one(std::make_shared<Message_Token_Insert>(child, network, sft, Node_Key_Symbol_1::Create(key_symbol->symbol), token));
  }

  void Node_Filter_1::remove_tokens(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> key, const std::shared_ptr<Node> child, const Filter_Layer_Snapshot snapshot) {
    const auto sft = shared_from_this();
    ...
    for (const auto &token : snapshot.snapshot<FILTER_LAYER_TOKENS>())
      job_queue->give_one(std::make_shared<Message_Token_Remove>(child, network, sft, Node_Key_Symbol_1::Create(key_symbol->symbol), token));
  }
  ...
  bool Node_Filter_1::operator==(const Node &rhs) const {
    ...
    if (auto filter_1 = dynamic_cast<const Node_Filter_1 *>(&rhs)) {
      return *get_key() == *filter_1->get_key() && get_input() == filter_1->get_input();
    }
