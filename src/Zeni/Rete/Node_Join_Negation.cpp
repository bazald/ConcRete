...
namespace Zeni::Rete {
  ...
  Node_Join_Negation::Node_Join_Negation(const std::shared_ptr<const Node_Key> node_key_left, const std::shared_ptr<const Node_Key> node_key_right, const std::shared_ptr<Node> input_left, const std::shared_ptr<Node> input_right, Variable_Bindings &&variable_bindings)
    : Node_Binary(std::max(input_left->get_height(), input_right->get_height()) + 1, input_left->get_size() + input_right->get_size() + 1, input_left->get_token_size() + input_right->get_token_size(), hash_combine(hash_combine(std::hash<int>()(2), node_key_left->hash()), node_key_right->hash()), node_key_left, node_key_right, input_left, input_right),
    m_variable_bindings(std::move(variable_bindings))
  {
  }

  std::shared_ptr<Node_Join_Negation> Node_Join_Negation::Create(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> node_key_left, const std::shared_ptr<const Node_Key> node_key_right, const std::shared_ptr<Node> input_left, const std::shared_ptr<Node> input_right, const Variable_Bindings &variable_bindings) {
    class Friendly_Node_Join_Negation : public Node_Join_Negation {
    public:
      Friendly_Node_Join_Negation(const std::shared_ptr<const Node_Key> node_key_left, const std::shared_ptr<const Node_Key> node_key_right, const std::shared_ptr<Node> input_left, const std::shared_ptr<Node> input_right, const Variable_Bindings variable_bindings) : Node_Join_Negation(node_key_left, node_key_right, input_left, input_right, variable_bindings) {}
    };

    const auto created = std::shared_ptr<Friendly_Node_Join_Negation>(new Friendly_Node_Join_Negation(node_key_left, node_key_right, input_left, input_right, variable_bindings));
  ...
  std::shared_ptr<Node_Join_Negation> Node_Join_Negation::Create(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> node_key_left, const std::shared_ptr<const Node_Key> node_key_right, const std::shared_ptr<Node> input_left, const std::shared_ptr<Node> input_right, Variable_Bindings &&variable_bindings) {
    class Friendly_Node_Join_Negation : public Node_Join_Negation {
    public:
      Friendly_Node_Join_Negation(const std::shared_ptr<const Node_Key> node_key_left, const std::shared_ptr<const Node_Key> node_key_right, const std::shared_ptr<Node> input_left, const std::shared_ptr<Node> input_right, Variable_Bindings &&variable_bindings) : Node_Join_Negation(node_key_left, node_key_right, input_left, input_right, std::move(variable_bindings)) {}
    };

    const auto created = std::shared_ptr<Friendly_Node_Join_Negation>(new Friendly_Node_Join_Negation(node_key_left, node_key_right, input_left, input_right, std::move(variable_bindings)));

    return connect_created(network, job_queue, input_left, input_right, created);
  }

  std::shared_ptr<Node_Join_Negation> Node_Join_Negation::connect_created(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<Node> input_left, const std::shared_ptr<Node> input_right, const std::shared_ptr<Node_Join_Negation> created) {
    const auto[result0, connected] = input_left->connect_new_or_existing_output(network, job_queue, created->get_key_left(), created);

    const auto connected_join = std::static_pointer_cast<Node_Join_Negation>(connected);
  ...
  std::pair<Node::Node_Trie::Result, std::shared_ptr<Node>> Node_Join_Negation::connect_new_or_existing_output(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> key, const std::shared_ptr<Node> child) {
  ...
  Node::Node_Trie::Result Node_Join_Negation::connect_existing_output(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> key, const std::shared_ptr<Node> child) {
    const auto sft = shared_from_this();
  ...
  bool Node_Join_Negation::has_tokens([[maybe_unused]] const std::shared_ptr<const Node_Key> key) const {
    assert(dynamic_cast<const Node_Key_Null *>(key.get()));
  ...
  void Node_Join_Negation::receive(const Message_Token_Insert &message) {
    const auto sft = shared_from_this();
  ...
  void Node_Join_Negation::receive(const Message_Token_Remove &message) {
    const auto sft = shared_from_this();
    const auto &job_queue = message.get_Job_Queue();
  ...
  void Node_Join_Negation::receive(const Message_Connect_Join &message) {
  ...
  void Node_Join_Negation::insert_tokens(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> key, const std::shared_ptr<Node> child, const Join_Layer_Snapshot snapshot) {
    const auto sft = shared_from_this();
  ...
  void Node_Join_Negation::remove_tokens(const std::shared_ptr<Network> network, const std::shared_ptr<Concurrency::Job_Queue> job_queue, const std::shared_ptr<const Node_Key> key, const std::shared_ptr<Node> child, const Join_Layer_Snapshot snapshot) {
    const auto sft = shared_from_this();
  ...
  std::shared_ptr<Symbols> Node_Join_Negation::bind_variables_left(const std::shared_ptr<const Token> token_left) const {
  ...
  std::shared_ptr<Symbols> Node_Join_Negation::bind_variables_right(const std::shared_ptr<const Token> token_right) const {
    auto symbols = std::make_shared<Symbols>();
    symbols->reserve(m_variable_bindings.size());
    for (auto variable_binding : m_variable_bindings)
  ...
  bool Node_Join_Negation::operator==(const Node &rhs) const {
    ...
    if (auto rhs_join = dynamic_cast<const Node_Join_Negation *>(&rhs)) {
