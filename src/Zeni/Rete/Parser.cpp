    ...
    namespace PEGTL {
      ...
      struct Action<Variable> {
        ...
        static void apply(const Input &input, Data &data) {
          ...
          data.symbols.push(std::pair<std::string_view, std::shared_ptr<Rete::Symbol>>(input.string(), nullptr));
        }
      };
      ...
      struct Action<Condition> {
        template<typename Input>
        static void apply(const Input &input, Data &data) {
          std::pair<std::string_view, std::shared_ptr<Rete::Symbol>> third = data.symbols.top();
          data.symbols.pop();
          std::pair<std::string_view, std::shared_ptr<Rete::Symbol>> second = data.symbols.top();
          data.symbols.pop();
          std::pair<std::string_view, std::shared_ptr<Rete::Symbol>> first = data.symbols.top();
          data.symbols.pop();
