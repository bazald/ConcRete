...
//    void execute() noexcept override {
//      while (m_to_insert) {
//        const uint64_t value = std::uniform_int_distribution<uint64_t>(1, 1000)(dre);
...
//        for (auto ss = snapshot.cbegin(), send = snapshot.cend(); ss != send; ++ss);
//        --m_to_lookup;
//      }
//      while (m_to_remove) {
//        const uint64_t value = std::uniform_int_distribution<uint64_t>(1, 1000)(dre);
//        m_hash_trie->erase_snapshot(value);
//        --m_to_remove;
//      }
//    }
//
...
//    int64_t m_to_insert = 256;
//    int64_t m_to_lookup = 256;
//    int64_t m_to_remove = 256;
//    std::random_device rd;
//    std::default_random_engine dre;
//  };
//
  ...
  void execute() noexcept override {
    ...
    while (!m_values_to_acquire.empty() || !m_values_to_release.empty()) {
      ...
      if (index < m_values_to_acquire.size()) {
        ...
        oss << std::this_thread::get_id() << " +" << *selected << ':';
#endif
        const auto[result, snapshot] = m_positive_hash_tries->template insert<MINE>(*selected);
        //snapshots.push_back(snapshot);
        ...
        oss << std::this_thread::get_id() << " -" << *selected << ':';
#endif
        const auto[result, snapshot] = m_positive_hash_tries->template erase<MINE>(*selected);
        //snapshots.push_back(snapshot);
