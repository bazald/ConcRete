...
namespace Zeni::Concurrency {
  ...
  public:
    ...
    public:
      ...
      const_iterator(std::mutex &mutex, const uint64_t earliest_epoch, const uint64_t current_epoch)
        : m_mutex_ptr(&mutex),
      ...
      const_iterator(std::mutex &mutex, const uint64_t earliest_epoch, const uint64_t current_epoch, Node * const node)
        : m_mutex_ptr(&mutex),
        m_earliest_epoch(earliest_epoch),
        m_current_epoch(current_epoch),
        m_node(node)
    ...
    const_iterator cbegin(const uint64_t earliest_epoch, const uint64_t current_epoch) const {
      ...
      return const_iterator(m_mutex, earliest_epoch, current_epoch, m_head);
    }

    const_iterator cend(const uint64_t earliest_epoch, const uint64_t current_epoch) const {
      std::lock_guard<std::mutex> lock(m_mutex);
      return const_iterator(m_mutex, earliest_epoch, current_epoch);
    }
