...
namespace Zeni::Rete {
  ...
  public:
    ZENI_RETE_LINKAGE Message_Connect_Output(const std::shared_ptr<Node> recipient, const std::shared_ptr<Network> network, const std::shared_ptr<const Node::Node_Data_Snapshot> snapshot, const std::shared_ptr<Node> child);

    ZENI_RETE_LINKAGE const std::shared_ptr<const Node> & get_sender() const;

    void receive() const override;

    const std::shared_ptr<const Node::Node_Data_Snapshot> snapshot;
